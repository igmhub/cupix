...
for iz, zbin in enumerate(out_AA_windowed):
    ...
    for itheta, theta in enumerate(zbin):
        if itheta>min_itheta:
            ...
            # plot the residuals
            # ax[1].plot(Px_data.k_AA[iz][is_nonzero], 
            #            (Px_data.Pk_AA[iz, itheta] / out_AA_windowed[iz][itheta])[is_nonzero]-1,
            #            linestyle=linestyle, color=color_set[itheta % len(color_set)])
            # ax[1].plot(Px_data.k_AA[iz][is_nonzero], 
            #            (Px_data.Pk_AA[iz, itheta] / out_AA_windowless[iz][itheta])[is_nonzero]-1,
...
np.diag(Px_data.cov_Pk_AA[iz,itheta]).shape
