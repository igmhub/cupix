...
fname = "/global/cfs/cdirs/desi/users/sindhu_s/Lya_Px_measurements/mocks/analysis-10/tru_cont/bf3_binned_out_px-zbins_4-thetabins_20_w_res_w_p1d.hdf5" # _w_skylines
mockdata = DESI_DR2(config={'data_file':fname, 'theta_min_cut_arcmin':30, 'kM_max_cut_AA':1, 'km_max_cut_AA':1.2})
# fname = ""
...
theory_config_xi = {'verbose': False, 'default_lya_model':'best_fit_arinyo_from_colore'}
theory_config_new = {'verbose': False, 'default_lya_model':'best_fit_arinyo_from_colore', 'q1':0, 'q2':0, 'kp':10000} # linear theory
for z in zs:
    theories_xi.append(Theory(z=z, fid_cosmo=cosmo, config=theory_config_xi))
    theories_new.append(Theory(z=z, fid_cosmo=cosmo, config=theory_config_new))
...
# %%
free_parameters = prepare_free_parameters(['bias', 'beta'], theories_new[0], theory_config_new)


# %%
for param in free_parameters:
    print(param.name, param.ini_value, param.min_value, param.max_value)

# %%
post = Posterior(likes_lya_new[0], free_parameters, config={'verbose': True})


# %%
mini = Minimizer(post=post)
...
# %%
mini.plot_ellipses(f"bias", f"beta", nsig=3, xrange=[-.139, -.103], yrange=[1.12,1.7], true_vals={'bias':theories_xi[iz].lya_model.default_lya_params['bias'], 'beta':theories_xi[iz].lya_model.default_lya_params['beta']}, true_val_label='Laura fit') 
...
mini.save_results(outfile=os.path.splitext(os.path.basename(fname))[0]+f"_iminuit_{zs[iz]}_skyline")

# %%
# check results
outfile = np.load(f"/global/common/software/desi/users/mlokken/cupix/data/fitter_results/bf3_binned_out_px-zbins_4-thetabins_20_w_res_w_p1d_iminuit_2.2_skyline.npz")
