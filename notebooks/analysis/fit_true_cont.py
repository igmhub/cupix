...
def plot_param(pname, runs, label):
    ...
    plt.tight_layout()
    plt.savefig('true_cont_{}_{}.png'.format(pname, label))
...
# %%
def plot_DNL(params={}, title=None):
    ...
    DNL_mu_0001 = theory._compute_DNL_Arinyo(k, mu=0.001, linP=linP, lya_params=lya_params)
    ...
    plt.semilogx(k, DNL_mu_0001, ls='--', label='mu = 0.001')
    plt.legend()
    plt.xlabel('k [1/Mpc]')
...
# %%
plot_DNL(params={}, title='Fit from Laura')

# %%
for run in runs_DNL:
    params = run['mini'].get_best_fit_params()
    plot_DNL(params=params, title=r'$\theta > {:.2f}$ arcmin'.format(run['theta_min']))

# %%
theory.lya_model.default_lya_params

# %%
mu = np.linspace(0,0.0001,1000)
