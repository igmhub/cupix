...
class IminuitMinimizer(object):
    ...
    def __init__(self, like, free_params, verbose=False):
        ...
        Np = len(free_params)
        for ip in range(Np):
            self.free_param_names.append(free_params[ip].name)
            self.ini_values.append(free_params[ip].ini_value)

        if self.verbose:
            print('Free parameters in minimizer')
            print(self.free_param_names)
            print('Inivial values set to', self.ini_values)

        # iminuit class itself
        self.minimizer = Minuit(self.minus_log_prob_interface, self.ini_values, name=self.free_param_names)
    ...
    def minus_log_prob_interface(self, values):
        ...
        params = self.get_params_dict_from_values(values)
    ...
    def get_best_fit_chi2(self, return_info=False):
        ...
        return self.like.get_chi2(params=best_fit_params, return_info=return_info)

    def get_best_fit_probability(self):
        # get best-fit parameters (will run minimizer if needed)
        best_fit_params = self.get_best_fit_params()
        return self.like.get_probability(best_fit_params, n_free_p=len(self.free_param_names))
