...
def read_from_file(datadir):
    ...
    with h5py.File(px_file, 'r') as f:
        ...
        px = np.zeros((ntheta, N))
        px_covar = np.zeros((ntheta, N, N))
        px_weights = np.zeros((ntheta, N))
        theta_bins = np.zeros((ntheta, 2))  # to store (theta_min, theta_max) in radians
        window = np.zeros((ntheta, N, N))  # window matrix for each theta
        for i, key in enumerate(theta_keys):
            ...
            # normalize and calculate the window matrix
            # (assuming all k are the same for every theta. If this changes, would need to update)
            window_matrix, norm = window_matrix_from_fft_weights(px_weights[i,:], R, L)
            window[i,:,:] = window_matrix
    ...
    return z, k, px, px_covar, px_weights, theta_bins, window
